...
def write_file_remote(hosts, ip, username, password):
    ...
    with open(hosts, 'w') as hosts:
        hosts.write("[remote] \n")
        hosts.write(
            ip + "\t ansible_ssh_user=" + username + "\t ansible_ssh_pass=" + password + "\t ansible_sudo_pass=" + password)
...
# 确认是否安装ansible & 确认在本地还是远端安装
if sys.version_info.major < 3:
    py2_input()
elif sys.version_info.major >=3:
    py3_input()

#TODO 验证输入的账号密码是否可以ssh登录，如果不可以询问用户：重新输入 or 退出？
